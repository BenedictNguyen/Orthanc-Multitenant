...
def on_create_worklist(uri, body):
...
def on_cleanup(uri, body):
    delete_old_studies()
    return json.dumps({"status": "ok", "msg": "Old studies cleanup done"})

orthanc.RegisterRestCallback("/ris/create-worklist", on_create_worklist)
orthanc.RegisterRestCallback("/ris/resend-study", on_resend_study)
orthanc.RegisterRestCallback("/ris/cleanup", on_cleanup)